...
class AfricaExtentError(Exception):
    """
    Exception to raise an error when a scene is outside Africa
    """
...
def start_transfer(message, valid_tile_ids):
    ...
    print(f"Task {s3_filepath} succeeded")
...
def copy_s3_objects(ti, **kwargs):
    ...
    for message in messages:
        try:
            ...
            failed += 1
        except AfricaExtentError as err:
            outside_extent += 1
            print(err)

    ti.xcom_push(key="successful", value=successful)
    ti.xcom_push(key="failed", value=failed)
    ti.xcom_push(key="outside_extent", value=outside_extent)
...
def trigger_sensor(ti, **kwargs):
    ...
    sqs = sqs_hook.get_resource_type("sqs")
    queue = sqs.get_queue_by_name(QueueName=default_args.get("sqs_queue"))
    messages = get_messages(queue, visibility_timeout=1)
...
def terminate(ti, **kwargs):
    ...
    for idx in range(0, default_args["num_workers"]):
        ...
        outside_africa_extent += ti.xcom_pull(
            key="outside_extent", task_ids=f"data_transfer_{idx}"
        )

    print(
        f"{successful_msg_counts} were successfully processed, {outside_africa_extent} were outside Africa extent and {failed_msg_counts} failed"
    )
